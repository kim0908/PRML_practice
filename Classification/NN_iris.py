...
if __name__ == '__main__':
    ...
    iris_data=np.array(iris_data.sample(frac=1.0))
    target=iris_data[:,-1]
    Target= pd.get_dummies(target).T.astype(np.float64).to_numpy()
    
    Data = iris_data[:,:-1].T.astype(np.float64)
    Train = Data[:,29:-1]
    Train_T = Target[:,29:-1]

    Val = Data[:,0:30]
    Val_T = Target[:,0:30]
    parameters,Loss_train,Loss_val ,Acc_train,Acc_val =nn_model(Train, Train_T,Val,Val_T, n_h=4, epoch = 200,learning_rate=0.3)
