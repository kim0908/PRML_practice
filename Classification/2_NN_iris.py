...
if __name__ == '__main__':
    ...
    Data = iris_data[:,:-1].T.astype(np.float64) #34*num
    Train = Data[:,29:-1]
    Train_T = Target[:,29:-1]
    ...
    plt.plot(Loss_train)
    plt.plot(Loss_val)
    plt.title('Iris_Loss lr={} nh={} ReLU'.format(learning_rate,n_h))
    ...
    plt.plot(Acc_train)
    plt.plot(Acc_val)
    plt.title('Iris_Accuracy lr={} nh={} ReLU'.format(learning_rate,n_h))
    plt.legend(['train', 'validation'])
    plt.show()
