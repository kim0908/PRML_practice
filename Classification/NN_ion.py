# -*- coding: utf-8 -*-
"""
Created on Thu May  6 12:43:29 2021
...
def softmax(x):
    ...
    total=0
    for i in range(len(x)):
        total+=np.exp(x[i]-np.max(x))
    for i in range(len(x)):
        exp_x = np.exp(x[i]-np.max(x))
        softmax_x[i] = exp_x / total
        if softmax_x[i] ==0:
            softmax_x[i]=softmax_x[i]+10**-6
    return softmax_x
'''
...
def compute_cost(Z2, Y, parameters):
    ...
    for n in range(len(Y.T)):
        for k in range(len(Y)):
            if Z2[k,n]==0:
                cross_ero.append(Y[k,n]*np.log(Z2[k,n]+10**-6))
            else:
                cross_ero.append(Y[k,n]*np.log(Z2[k,n]))
    loss = -(np.sum(cross_ero))   
    return loss
...
def nn_model(X, Y,X_val, Y_val,n_h, epoch = 200, learning_rate=0.3):
    np.random.seed(3)
...
if __name__ == '__main__':
    ...
    ionosphere_data = pd.read_csv("ionosphere_data.csv",header=None).to_numpy()  
    target=ionosphere_data[:,-1]
    Target= pd.get_dummies(target).T.astype(np.float64).to_numpy() # g:01,b:10 #2*num
    Data = ionosphere_data[:,:-1].T.astype(np.float64) #34*num
    Train = Data[:,69:-1]
    Train_T = Target[:,69:-1]
    #Train,Train_T=shuffle(Train,Train_T)
    Val = Data[:,0:70]
    Val_T = Target[:,0:70]
    parameters,Loss_train,Loss_val ,Acc_train,Acc_val =nn_model(Train, Train_T,Val,Val_T, n_h=10, epoch = 200,learning_rate=0.3)

    
    plt.figure()
    plt.plot(Loss_train)
    plt.plot(Loss_val)
    plt.title('ionosphere_Loss')
